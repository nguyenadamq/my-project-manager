      ...
      <section className="panel"><div className="section-head"><div><span className="eyebrow">Supervised agent work</span><h2>Prompt queue</h2></div><span className="count">{project.queue?.filter((item) => item.status === "queued").length ?? 0}</span></div>
        <form className="prompt-form" onSubmit={addPrompt}><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Describe the next change…"/>
          ...
          <fieldset className="mode-select"><legend>When to run</legend>
            <label><input type="radio" name="dispatch" checked={dispatch === "queued"} onChange={() => setDispatch("queued")}/> Queue it <small>{mode === "full" ? "Plans right away (read-only) so it's ready to review — implementation still waits for your approval, or for auto-dispatch to approve it once the agents have usage headroom." : "Waits its turn. Start it yourself, or let auto-dispatch pick it up across projects by priority as soon as the agents have usage headroom."}</small></label>
